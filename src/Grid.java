...
public class Grid extends Canvas {
	...
	public void paint(Graphics g) {
		...
		Color[] c = { Color.white, Color.white, new Color(255, 255, 170), new Color(255, 255, 128),
				new Color(255, 255, 85), new Color(255, 255, 43), new Color(255, 255, 0), new Color(213, 213, 0),
				new Color(170, 170, 0), new Color(128, 128, 0), new Color(85, 85, 0), new Color(43, 43, 0) };
		...
		if (game.context.state.getCurrentState() == 1) {// 2 == gameoverstate
			...
			for (int i = 0; i < rows; i++) {
				for (int j = 0; j < cols; j++) {
					if (game.board.board[i][j] != 0) {
						...
						int n = (int) (Math.log(value) / Math.log(2));// get the
																		// number
																		// in
																		// form
																		// of
			...
			g.setFont(new Font("Default", Font.PLAIN, sizeFont));
			;
			g.drawString("Your Score:", width + 40, sizeFont * 2);
			g.drawString(Integer.toString(((Score) game.observers.get(0)).score), width + 40, sizeFont * 4);
			...
			g.drawString("Game Over!", width / 2 + 4, height / 2 - 60 + 4);
			g.setColor(Color.white);
			g.drawString("Game Over!", width / 2, height / 2 - 60);
			g.setFont(new Font("Purisa", Font.PLAIN, 15));
			g.drawString("Your Score: " + ((Score) game.observers.get(0)).score, width / 2, height / 2 + 10);
			g.drawString("Press space to go to the menu!", width / 2, height / 2 + 40);// TODO
																						// not
																						// sure
																						// if
																						// this
